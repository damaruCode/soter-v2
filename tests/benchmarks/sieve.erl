...
%%
%%     Reference: Rob Pike, [Concurrency and Message Passing In Newsqueak](http://video.google.com/videoplay?docid=810232012617965344)
%%
...
filter(Test, In, Out) ->
    In ! {poke, self()}, % Duplicating this line makes the program faster but the mailboxes become unbounded!
    receive
        {ans,Y} ->
            case Test(Y) of
                false -> Out ! {ans, Y}, filter(Test, In);
                _  -> filter(Test, In, Out)
            end
    end.
