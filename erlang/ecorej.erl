...
to_corej(Module) ->
  case compile:file(Module, [to_core, binary, no_copt]) of
    {ok, _, Ast} ->
      io:put_chars(Json = jsx:encode(enc(Ast))),
...
enc({c_bitstr, Anno, Val, Size, Unit, Type, Flags}) ->
  jsx:encode(#{<<"type">> => c_bitstr,
               ...
               <<"flags">> => enc(Flags)});
% -record(c_call, {anno = [] :: list(), module :: cerl:cerl(), name :: cerl:cerl(), args :: [cerl:cerl()]}).
enc({c_call, Anno, Module, Name, Args}) ->
  jsx:encode(#{<<"type">> => c_call,
               <<"anno">> => enc(Anno),
               <<"module">> => enc(Module),
               <<"name">> => enc(Name),
               <<"args">> => enc(Args)});
...
enc({c_case, Anno, Arg, Clauses}) ->
  jsx:encode(#{<<"type">> => c_case,
               <<"anno">> => enc(Anno),
               <<"arg">> => enc(Arg),
               <<"clauses">> => enc(Clauses)});
% -record(c_catch, {anno = [] :: list(), body :: cerl:cerl()}).
enc({c_catch, Anno, Body}) ->
  jsx:encode(#{<<"type">> => c_catch,
               <<"anno">> => enc(Anno),
               <<"body">> => enc(Body)});
% -record(c_clause, {anno = [] :: list(), pats :: [cerl:cerl()], guard :: cerl:cerl(), body :: cerl:cerl() | any()}).
enc({c_clause, Anno, Pats, Guard, Body}) ->
  jsx:encode(#{<<"type">> => c_clause,
               <<"anno">> => enc(Anno),
               <<"pats">> => enc(Pats),
               <<"guard">> => enc(Guard),
               <<"body">> => enc(Body)});
...
enc({c_cons, Anno, Hd, Tl}) ->
  jsx:encode(#{<<"type">> => c_cons,
               <<"anno">> => enc(Anno),
               <<"hd">> => enc(Hd),
               <<"tl">> => enc(Tl)});
% -record(c_fun, {anno = [] :: list(), vars :: [cerl:cerl()], body :: cerl:cerl()}).
enc({c_fun, Anno, Vars, Body}) ->
  jsx:encode(#{<<"type">> => c_fun,
               <<"anno">> => enc(Anno),
               <<"vars">> => enc(Vars),
               <<"body">> => enc(Body)});
% -record(c_let, {anno = [] :: list(), vars :: [cerl:cerl()], arg :: cerl:cerl(), body :: cerl:cerl()}).
enc({c_let, Anno, Vars, Arg, Body}) ->
  jsx:encode(#{<<"type">> => c_let,
               <<"anno">> => enc(Anno),
               <<"vars">> => enc(Vars),
               <<"arg">> => enc(Arg),
               <<"body">> => enc(Body)});
% -record(c_letrec, {anno = [] :: list(), defs :: [{cerl:cerl(), cerl:cerl()}], body :: cerl:cerl()}).
enc({c_letrec, Anno, Defs, Body}) ->
  jsx:encode(#{<<"type">> => c_letrec,
               <<"anno">> => enc(Anno),
               <<"defs">> => enc(Defs),
               <<"body">> => enc(Body)});
% -record(c_literal, {anno = [] :: list(), val :: any()}).
enc({c_literal, Anno, Val}) ->
  jsx:encode(#{<<"type">> => c_literal,
               <<"anno">> => enc(Anno),
               <<"val">> => enc(Val)});
% -record(c_map, {anno = [] :: list(), arg = #c_literal{val = #{}}, es :: [cerl:c_map_pair()], is_pat = false :: boolean()}).
enc({c_map, Anno, Arg, Es, IsPat}) ->
  jsx:encode(#{<<"type">> => c_map,
               <<"anno">> => enc(Anno),
               <<"arg">> => enc(Arg),
               <<"es">> => enc(Es),
               <<"is_pat">> => enc(IsPat)});
% -record(c_map_pair, {anno = [] :: list(), op :: #c_literal{val :: assoc} | #c_literal{val :: exact}, key :: any(), val :: any()}).
enc({c_map_pair, Anno, Op, Key, Val}) ->
  jsx:encode(#{<<"type">> => c_map_pair,
               <<"anno">> => enc(Anno),
               <<"op">> => enc(Op),
               <<"key">> => enc(Key),
               <<"val">> => enc(Val)});
% -record(c_module, {anno = [] :: list(), name :: cerl:cerl(), exports :: [cerl:cerl()], attrs :: [{cerl:cerl(), cerl:cerl()}], defs :: [{cerl:cerl(), cerl:cerl()}]}).
enc({c_module, Anno, Name, Exports, Attrs, Defs}) ->
  jsx:encode(#{<<"type">> => c_module,
               <<"anno">> => enc(Anno),
               <<"name">> => enc(Name),
               <<"exports">> => enc(Exports),
               <<"attrs">> => enc(Attrs),
...
enc({c_opaque, Anno, Val}) ->
  jsx:encode(#{<<"type">> => c_opaque,
               <<"anno">> => enc(Anno),
               <<"val">> => enc(Val)});
% -record(c_primop, {anno = [] :: list(), name :: cerl:cerl(), args :: [cerl:cerl()]}).
enc({c_primop, Anno, Name, Args}) ->
  jsx:encode(#{<<"type">> => c_primop,
               <<"anno">> => enc(Anno),
               <<"name">> => enc(Name),
               <<"args">> => enc(Args)});
% -record(c_receive, {anno = [] :: list(), clauses :: [cerl:cerl()], timeout :: cerl:cerl(), action :: cerl:cerl()}).
enc({c_receive, Anno, Clauses, Timeout, Action}) ->
  jsx:encode(#{<<"type">> => c_receive,
               <<"anno">> => enc(Anno),
               <<"clauses">> => enc(Clauses),
               <<"timeout">> => enc(Timeout),
               <<"action">> => enc(Action)});
% -record(c_seq, {anno = [] :: list(), arg :: cerl:cerl() | any(), body :: cerl:cerl()}).
enc({c_seq, Anno, Arg, Body}) ->
  jsx:encode(#{<<"type">> => c_seq,
               <<"anno">> => enc(Anno),
               <<"arg">> => enc(Arg),
               <<"body">> => enc(Body)});
% -record(c_try, {anno = [] :: list(), arg :: cerl:cerl(), vars :: [cerl:cerl()], body :: cerl:cerl(), evars :: [cerl:cerl()], handler :: cerl:cerl()}).
enc({c_try, Anno, Arg, Vars, Body, EVars, Handler}) ->
  jsx:encode(#{<<"type">> => c_try,
               <<"anno">> => enc(Anno),
               <<"arg">> => enc(Arg),
               <<"vars">> => enc(Vars),
               <<"body">> => enc(Body),
